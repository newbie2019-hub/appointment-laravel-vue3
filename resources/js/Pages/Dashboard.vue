...
const labels = computed(() => {
    return props.chartServices.map((service) => service.service);
});

const series = computed(() => {
    return props.chartServices.map(
        (service) => service.monthly_appointments_count
    );
});

const areaChartLabel = computed(() => {
    return props.appointmentsIncome.map((income) => income.date);
});

const areaData = computed(() => {
    return props.appointmentsIncome.map((income) => String(income.subtotal));
});
...
let summaryDate = ref(props.filters.summaryDate);
let appointmentDate = ref(props.filters.appointmentDate);
...
const searchAppointment = debounce(() => {
    Inertia.get(
        ...
        {
            preserveState: true,
            only: ["appointments", "appointmentsIncome", "chartServices"],
        }
    );
}, 500);
</script>
...
<template>
    ...
    <BreezeAuthenticatedLayout>
        <div class="px-6 max-w-8xl lg:px-8">
            ...
            >
                <div class="p-6">
                    ...
                    <p class="text-sm text-gray-600">
                        Shown below are the summary of your transactions and
                        records.
                    </p>
                </div>
            </div>
            <div
                class="flex flex-wrap max-w-8xl sm:px-6 lg:px-8 gap-y-4 gap-x-4"
                v-if="$page.props.auth.user.is_admin"
            >
                <div class="flex-1 p-4 text-white bg-green-600 rounded-md">
                    <div class="flex justify-between">
                        <div class="flex items-center text-9xl">
                            {{ formatNumeric(servicesCount) }}
                        </div>
                ...
                <div class="md:flex gap-x-2">
                    <div class="flex-1 w-full md:w-1/2">
                        <div class="mt-8">
                            ...
                            <div
                                class="mt-4 mb-3 flex justify-end items-center"
                            >
                                <p class="text-sm mr-3">Filter Date:</p>
                                <Datepicker
                                    v-model="summaryDate"
                                    range
                    ...
                    <div class="flex-1 w-full md:w-1/2">
                        ...
                        <div class="mt-4 mb-3 flex justify-end items-center">
                            <p class="text-sm mr-3">Filter Date:</p>
                            <Datepicker
            ...
            <div
                class="pb-6 mt-10 bg-white border-gray-200 rounded-lg max-w-8xl sm:px-6 lg:px-8"
            >
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xl font-medium">Latest Appointments</p>
                        <p class="text-sm text-gray-700">
                            Shown below are the latest appointments.
                        </p>
                    </div>

                    <Link
                        :href="route('appointments.index')"
                        class="flex items-center font-medium transition duration-200 ease-in-out hover:translate-x-1 hover:text-blue-500 gap-x-1"
                    >
                        View All
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                    ...
                    <table class="min-w-full divide-y divide-gray-300">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr
                                ...
                                :class="{
                                    'bg-red-100': appointment.deleted_at,
                                }"
                            >
                                <td
                                ...
                                <td
                                    class="py-4 pl-4 pr-3 text-sm font-medium text-gray-900 whitespace-nowrap sm:pl-6 gap-x-2"
                                >
                                    <div class="flex items-center gap-x-2">
                                        <div
                                            class="flex items-center justify-center w-8 h-8 bg-blue-500 border-gray-100 rounded-full"
                                        >
                                            <svg
                                                v-if="
                                            ...
                                            <img
                                                :src="`images/profile/${appointment.patient?.image}`"
                                                class="h-full rounded-full object-fit"
                                                alt=""
                                            />
                                        </div>
                                ...
                                >
                                    {{ stringLimit(appointment.message) }}
                                </td>
                                <td
                                    class="py-4 pl-4 pr-3 text-sm font-medium text-gray-900 whitespace-nowrap sm:pl-6"
                                >
                ...
                <p class="mt-2 text-sm text-gray-500">
                    Showing the latest entry of appointments from patients.
                </p>
            </div>

            <Modal
                v-if="isAppointmentModalShown"
                @close="toggleAppointmentModal"
            >
            ...
            <Modal v-if="isDeleteModalShown" @close="toggleDeleteModal">
                ...
                <template v-slot:body>
                    <p class="text-sm text-gray-600">
                        ...
                        Once moved to trash the appointment status will be
                        marked as cancelled and cannot be restored.
                    </p>
                </template>
                <template v-slot:footer>
                    <Button
